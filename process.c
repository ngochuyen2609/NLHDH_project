...
void dungTienTrinh(DWORD pid)
{
    for (int i = 0; i < soTienTrinh; i++)
    {
        if (dsTienTrinh[i].pid == pid)
        {
            ...
            if (NtSuspendProcess && NtSuspendProcess(hProcess) == 0)
            {                              // NtSuspendProcess suspends the process
...
void tiepTucTienTrinh(DWORD pid)
{
    for (int i = 0; i < soTienTrinh; i++)
    {
        if (dsTienTrinh[i].pid == pid)
        {
            ...
            if (NtResumeProcess && NtResumeProcess(hProcess) == 0)
            {                              // NtResumeProcess resumes the process
...
void thucThiTienTrinh(char **thamSo, int cheDoNen)
{
    ...
    {
        fprintf(stderr, "⚠ Can't execute this command '%s'. Lỗi: %lu\n", fullPath, GetLastError());
    }
}
