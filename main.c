    ...
    while (1)
    {
        ...
        else if (strncmp(lenh, "kill ", 5) == 0)
        {
            ...
            if (pid > 0)
                ...
                printf("Invalid PID for kill command.\n");
        }
        else if (strncmp(lenh, "stop ", 5) == 0)
        {
            ...
            if (pid > 0)
                ...
                printf("Invalid PID for stop command.\n");
        }
        else if (strncmp(lenh, "resume ", 7) == 0)
        {
            int pid = atoi(lenh + 7);
            if (pid > 0)
                tiepTucTienTrinh(pid);
            else
                printf("Invalid PID for resume command.\n");
        }
            ...
            inNgayGio();
        else if (strstr(lenh, ".bat") != NULL)
            thucHienFileBat(lenh);
        else
        {
